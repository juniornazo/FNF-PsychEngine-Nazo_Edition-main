...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in 0...optionShit.length)
		{
			...
			if (ClientPrefs.languaGame == 'English') menuItem.frames = Paths.getSparrowAtlas('mainmenu/menu_' + optionShit[i]);
			else if (ClientPrefs.languaGame == 'Spanish') menuItem.frames = Paths.getSparrowAtlas('mainmenuESP/menu_' + optionShit[i]);
			else if (ClientPrefs.languaGame == 'Portuguese') menuItem.frames = Paths.getSparrowAtlas('mainmenuPTBR/menu_' + optionShit[i]);
		...
		var versionShit:FlxText = new FlxText(1098, 0, 0, "Psych Engine v" + psychEngineVersion, 12);
		...
		add(versionShit);
		var versionShit:FlxText = new FlxText(1023, 15, 0, "Friday Night Funkin' v" + Application.current.meta.get('version'), 12);
		versionShit.scrollFactor.set();
		#if desktop
		versionShit.x += 8;
		#end
				...
				if (FlxG.mouse.overlaps(firstPrize))
					{
						...
						FlxTween.tween(rollWrench, {alpha: 0}, 0.4, {
						...
						FlxTween.tween(bumpGame, {alpha: 0}, 0.4, {
							ease: FlxEase.quadOut,
							onComplete: function(twn:FlxTween)
							{
								bumpGame.kill();
						...
						FlxFlicker.flicker(firstPrize, 1, 0.06, false, false, function(flick:FlxFlicker)
							{
								MusicBeatState.switchState(new AchievementsMenuState());
							});
				...
				else if (FlxG.mouse.overlaps(rollWrench))
					{
						...
						FlxTween.tween(firstPrize, {alpha: 0}, 0.4, {
							ease: FlxEase.quadOut,
							onComplete: function(twn:FlxTween)
							{
								rollWrench.kill();
						...
						FlxFlicker.flicker(rollWrench, 1, 0.06, false, false, function(flick:FlxFlicker)
							{
								MusicBeatState.switchState(new options.OptionsState());
							});
